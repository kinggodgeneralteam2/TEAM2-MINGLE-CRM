...
import com.team2final.minglecrm.repository.CustomerRepository;
...
public class CustomerServiceImpl implements CustomerService {
    ...
    public List<CustomerListResponseDTO> getCustomersByUserId(String userName) {
        return customerRepository.findByUserName(userName).stream()
                .map(customer -> new CustomerListResponseDTO(
                        customer.getId(),
                        customer.getName(),
                        customer.getCustomer_group(),
    ...
    public List<CustomerListResponseDTO> getCustomersByCustomerGroup(String customerGroup) {
        return customerRepository.findByCustomer_group(customerGroup).stream() // 여기염
                .map(customer -> new CustomerListResponseDTO(
                        customer.getId(),
                        customer.getName(),
                        customer.getCustomer_group(),
